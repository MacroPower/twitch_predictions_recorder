...
type DB interface {
	...
	GetRelatedSummaries(string, int) ([]models.EventSummary, string, error)
}
...
func (gdb *GormDB) GetRelatedSummaries(title string, distance int) ([]models.EventSummary, string, error) {
	...
	query := session.
		...
		Where("ldist(?, events.title) < ?", title, distance)
